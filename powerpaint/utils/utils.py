...
def add_tokens(
    ...
    # get exteral_embedding of embedding_layer if have
    external_embeddings = {}
    for emb in embedding_layer.external_embeddings:
        external_embeddings[emb['name']] = emb['embedding']

    if initialize_tokens is not None:
        for ii in range(len(placeholder_tokens)):
            if initialize_tokens[ii] not in external_embeddings.keys():
                ...
                temp_embedding = external_embeddings[initialize_tokens[ii]]
                initialize_embedding.append(temp_embedding)
    else:
        for ii in range(len(placeholder_tokens)):
