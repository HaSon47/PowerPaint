...
def get_inputs(img_folder_path, root_path='/mnt/disk2/hachi/data/workdir/radish/hachi/OBJ_INS/phase2_V2'):
    print('get input')
    inputs = {}
    for img_file in tqdm(os.listdir(img_folder_path)):
        ...
        if os.path.exists(os.path.join(root_path, 'test', img_name, 'annotation.json')):
            anno_path = os.path.join(root_path, 'test', img_name, 'annotation.json')
        else:
            anno_path = os.path.join(root_path,'val', img_name, 'annotation.json')
        
        with open(anno_path, 'r') as f:
            anno = json.load(f)
        
        ## get class
        class_prompt = anno['class_based_caption']

        ## get bbox
        bbox = anno['inpainted_bboxes'][0]  # [x_min, y_min, x_max, y_max]
        img_width, img_height = image.size
        # expand bbox by 10% on each side
        width = bbox[2] - bbox[0]
        ...
        bbox = [
            ...
            min(img_width, int(bbox[2] + expand_x)),
            min(img_height, int(bbox[3] + expand_y))
        ]
